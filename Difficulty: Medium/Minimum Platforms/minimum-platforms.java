...
class Solution {
    ...
    static int findPlatform(int arr[], int dep[]) {
        // add your code here
        PriorityQueue<int[]> platforms = new PriorityQueue<>((a,b) -> a[0] - b[0]);
        int n = dep.length;
        int minPlatforms = 1;
        for(int i = 0 ; i < n ; i++){
            platforms.add(new int[]{arr[i],dep[i]});
        }
        PriorityQueue<Integer> pq = new PriorityQueue<>();
        int[] platform = platforms.poll();
        pq.add(platform[1]);
        while(!platforms.isEmpty()){
            platform = platforms.poll();
            // System.out.println(Arrays.toString(platform));
            if(!pq.isEmpty()){
                int currDept = pq.poll();
                if(platform[0] > currDept){
                    pq.add(platform[1]);
                }
                else{
                    pq.add(currDept);
                ...
                minPlatforms = Math.max(minPlatforms,pq.size());
            }
        }
        return minPlatforms;
    }
